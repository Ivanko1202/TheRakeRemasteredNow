...
if game.GameId == 847722000 then
	...
	pcall(function() getgenv().PyDeathGui = true end)
	...
	local Window = Rayfield:CreateWindow({
		...
		LoadingSubtitle = "By PyDeath Hub",
		ConfigurationSaving = {
			Enabled = true,
			...
			elseif _G.RakeChams == false then
				if game:GetService("Workspace"):FindFirstChild("Rake") then
					if game:GetService("Workspace"):FindFirstChild("Rake"):FindFirstChild("RakeChams") then
						game:GetService("Workspace"):FindFirstChild("Rake"):FindFirstChild("RakeChams"):Remove()
					end
				end
			end
		end,
